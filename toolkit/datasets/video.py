...
class Video(object):
    def __init__(self, name, root, video_dir, init_rect, img_names,
        ...
        if os.path.basename(root) == 'DTB70':
            self.img_names = [os.path.join(root, x) for x in img_names]
        
        if os.path.basename(root) == 'UAV123':
            self.img_names = [os.path.join(root, 'Dataset_UAV123/UAV123/data_seq/UAV123', x) for x in img_names]

        if os.path.basename(root) in ['UAV123@10fps', 'UAV12320l']:
            self.img_names = [os.path.join(root, 'data_seq', x) for x in img_names]
        
        if os.path.basename(root) == 'UAVDT':
            self.img_names = [os.path.join(root, x) for x in img_names]

        if os.path.basename(root) == 'UAVTrack112':
            self.img_names = [os.path.join(root, x) for x in img_names]
